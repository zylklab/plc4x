...
|CD20 |The project's code is easily discoverable and publicly accessible. |icon:check[role=green] Our source code is available at https://gitbox.apache.org/repos/asf/plc4x.git[Apache GitBox] and https://github.com/apache/plc4x[GitHub] and linked to from our https://plc4x.apache.org/scm.html[website].
...
|CS20 |Decisions are made by consensus among PMC members 9 and are documented on the project's main communications channel. Community opinions are taken into account but the PMC has the final word if needed. |icon:check[role=green] All decisions are made on one of our mailing lists. Every decision discussed off-list has been taken back to the list for final discussion and we will keep on doing that.
